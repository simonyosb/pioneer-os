...
import { agentApiKeys, agents, authUsers, companies, companyMemberships, instanceUserRoles } from "@ardonex/db";
...
export function actorMiddleware(db: Db, opts: ActorMiddlewareOptions): RequestHandler {
  ...
  return async (req, _res, next) => {
    ...
    // Accept both the new X-Ardonex-Run-Id header and the legacy X-Paperclip-Run-Id header.
    // X-Ardonex-Run-Id takes precedence; X-Paperclip-Run-Id is accepted for backward compat.
    const runIdHeader = req.header("x-ardonex-run-id") ?? req.header("x-paperclip-run-id");
