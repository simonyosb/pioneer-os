...
import type { CreateIssueThreadInteraction, IssueDocumentSummary } from "@pioneeros/shared";
...
export function buildHostServices(
  ...
  options: { pluginWorkerManager?: PluginWorkerManager; manifest?: import("@pioneeros/shared").PaperclipPluginManifestV1 } = {},
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@pioneeros/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
