...
- **Hiring**: use the `pioneeros-create-agent` skill for new agent creation workflows (links to reusable `AGENTS.md` templates like `Coder` and `QA`).
- **Commit Co-author**: if you make a git commit you MUST add EXACTLY `Co-Authored-By: Pioneer OS <noreply@pioneeros.com>` to the end of each commit message. Do not put in your agent name, put `Co-Authored-By: Pioneer OS <noreply@pioneeros.com>`.
...
When asked to convert a plan into executable Paperclip tasks — depth, assignment, dependencies, parallelization — use the companion skill `pioneeros-converting-plans-to-tasks`.
