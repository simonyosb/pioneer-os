...
// Detect if the binary was invoked as "paperclipai" (the legacy name) and emit
// a one-time deprecation banner. The banner is suppressed when ARDONEX_NO_DEPRECATION_BANNER=1.
const invocationName = path.basename(process.argv[1] ?? "");
const isLegacyInvocation =
  (invocationName === "paperclipai" || invocationName.startsWith("paperclipai.")) &&
  process.env.ARDONEX_NO_DEPRECATION_BANNER !== "1";
if (isLegacyInvocation) {
  process.stderr.write(
    "[ardonex] DeprecationWarning: The 'paperclipai' command is deprecated. Use 'ardonex' instead. " +
...
const DATA_DIR_OPTION_HELP =
  "Ardonex data directory root (isolates state from ~/.ardonex)";

program
  .name("pioneer")
  .description("Ardonex CLI — setup, diagnose, and configure your instance")
  .version(cliVersion);
...
program
  ...
  .option("--run", "Start Ardonex immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Ardonex setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "ardonex")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Ardonex")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Ardonex server API")
