 ...
 *
 * Bundles all workspace packages (@pioneeros/*) into a single file.
...
// to npm and resolved at runtime (e.g. @pioneeros/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@pioneeros/")) {
      externals.add(name);
    }
  }
