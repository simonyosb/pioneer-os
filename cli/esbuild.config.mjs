 ...
 *
 * Bundles all workspace packages (@ardonex/*) into a single file.
...
// to npm and resolved at runtime (e.g. @ardonex/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@ardonex/")) {
      externals.add(name);
    }
  }
