...
## 2. Shared Types (`@ardonex/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@ardonex/adapter-utils` (types) or `@ardonex/adapter-utils/server-utils` (runtime helpers).
...
```json
{
  ...
  "dependencies": {
    "@ardonex/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@ardonex/adapter-utils/server-utils`)
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@ardonex/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@ardonex/adapter-my-agent";
...
import { parseMyAgentStdoutLine } from "@ardonex/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@ardonex/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@ardonex/adapter-my-agent/cli";
...
Import from `@ardonex/adapter-utils/server-utils`:
...
- Package name: `@ardonex/adapter-<kebab-name>`
...
- Default prompt should use `DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE` from `@ardonex/adapter-utils/server-utils` so local adapters share Paperclip's execution contract: act in the same heartbeat, avoid planning-only exits unless requested, leave durable progress and a next action, use child issues instead of polling, mark blockers with owner/action, and respect governance boundaries.
