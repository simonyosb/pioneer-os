...
- The host provides a small shared React component kit through
  `@pioneeros/plugin-sdk/ui`; use it for common Paperclip controls before
...
```bash
pnpm --filter @pioneeros/create-paperclip-plugin build
...
Inside this monorepo, the scaffold uses `workspace:*` for `@pioneeros/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@pioneeros/plugin-sdk` from the local Paperclip checkout into a `.paperclip-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@pioneeros/plugin-sdk/ui`
...
Use shared components from `@pioneeros/plugin-sdk/ui` when the plugin needs a
...
```tsx
import { AssigneePicker, ProjectPicker } from "@pioneeros/plugin-sdk/ui";
...
Use `FileTree` from `@pioneeros/plugin-sdk/ui` whenever the plugin only needs
